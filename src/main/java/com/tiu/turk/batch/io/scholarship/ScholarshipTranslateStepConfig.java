package com.tiu.turk.batch.io.scholarship;
...
import com.tiu.turk.batch.io.scholarship.processor.ScholarshipProcessorComposite;
import com.tiu.turk.batch.io.scholarship.writer.ScholarshipTranslationWriter;
...
public class ScholarshipTranslateStepConfig {
    private final JobRepository jobRepository;
    private final PlatformTransactionManager tx;
    private final BackOffPolicy exponentialBackoffPolicy;
    private final TaskReaderFactory taskReaderFactory;
    private final ScholarshipTranslationWriter scholarshipTranslationWriter;
    private final ScholarshipProcessorComposite scholarshipCompositeProcessor;
    private final FailureListenerFactory listenerFactory;

    @Bean
    public Step translateScholarshipStep() {
        return new StepBuilder("translateScholarshipStep", this.jobRepository)
                .<TranslationTaskEntity, TranslationResult>chunk(20, this.tx)
                .reader(this.taskReaderFactory.getDefaultRepositoryReader(AppModule.SCHOLARSHIP))
                .processor((ItemProcessor<TranslationTaskEntity, TranslationResult>) this.scholarshipCompositeProcessor)
                .writer(this.scholarshipTranslationWriter)
                .faultTolerant()
