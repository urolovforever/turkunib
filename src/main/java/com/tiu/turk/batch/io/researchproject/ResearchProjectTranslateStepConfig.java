package com.tiu.turk.batch.io.researchproject;
...
import com.tiu.turk.batch.io.researchproject.processor.ResearchProjectProcessorComposite;
import com.tiu.turk.batch.io.researchproject.writer.ResearchProjectTranslationWriter;
...
public class ResearchProjectTranslateStepConfig {
    private final JobRepository jobRepository;
    private final PlatformTransactionManager tx;
    private final BackOffPolicy exponentialBackoffPolicy;
    private final TaskReaderFactory taskReaderFactory;
    private final ResearchProjectTranslationWriter researchProjectTranslationWriter;
    private final ResearchProjectProcessorComposite researchProjectCompositeProcessor;
    private final FailureListenerFactory listenerFactory;

    @Bean
    public Step translateResearchProjectStep() {
        return new StepBuilder("translateResearchProjectStep", this.jobRepository)
                .<TranslationTaskEntity, TranslationResult>chunk(20, this.tx)
                .reader(this.taskReaderFactory.getDefaultRepositoryReader(AppModule.RESEARCH_PROJECT))
                .processor((ItemProcessor<TranslationTaskEntity, TranslationResult>) this.researchProjectCompositeProcessor)
                .writer(this.researchProjectTranslationWriter)
                .faultTolerant()
