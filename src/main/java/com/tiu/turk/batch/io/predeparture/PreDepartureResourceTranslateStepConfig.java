package com.tiu.turk.batch.io.predeparture;
...
import com.tiu.turk.batch.io.predeparture.processor.PreDepartureResourceProcessorComposite;
import com.tiu.turk.batch.io.predeparture.writer.PreDepartureResourceTranslationWriter;
...
public class PreDepartureResourceTranslateStepConfig {
    private final JobRepository jobRepository;
    private final PlatformTransactionManager tx;
    private final BackOffPolicy exponentialBackoffPolicy;
    private final TaskReaderFactory taskReaderFactory;
    private final PreDepartureResourceTranslationWriter preDepartureResourceTranslationWriter;
    private final PreDepartureResourceProcessorComposite preDepartureResourceCompositeProcessor;
    private final FailureListenerFactory listenerFactory;

    @Bean
    public Step translatePreDepartureResourceStep() {
        return new StepBuilder("translatePreDepartureResourceStep", this.jobRepository)
                .<TranslationTaskEntity, TranslationResult>chunk(20, this.tx)
                .reader(this.taskReaderFactory.getDefaultRepositoryReader(AppModule.PRE_DEPARTURE_RESOURCE))
                .processor((ItemProcessor<TranslationTaskEntity, TranslationResult>) this.preDepartureResourceCompositeProcessor)
                .writer(this.preDepartureResourceTranslationWriter)
                .faultTolerant()
