package com.tiu.turk.batch.io.webinar;
...
import com.tiu.turk.batch.io.webinar.processor.WebinarProcessorComposite;
import com.tiu.turk.batch.io.webinar.writer.WebinarTranslationWriter;
...
public class WebinarTranslateStepConfig {
    private final JobRepository jobRepository;
    private final PlatformTransactionManager tx;
    private final BackOffPolicy exponentialBackoffPolicy;
    private final TaskReaderFactory taskReaderFactory;
    private final WebinarTranslationWriter webinarTranslationWriter;
    private final WebinarProcessorComposite webinarCompositeProcessor;
    private final FailureListenerFactory listenerFactory;

    @Bean
    public Step translateWebinarStep() {
        return new StepBuilder("translateWebinarStep", this.jobRepository)
                .<TranslationTaskEntity, TranslationResult>chunk(20, this.tx)
                .reader(this.taskReaderFactory.getDefaultRepositoryReader(AppModule.WEBINAR))
                .processor((ItemProcessor<TranslationTaskEntity, TranslationResult>) this.webinarCompositeProcessor)
                .writer(this.webinarTranslationWriter)
                .faultTolerant()
