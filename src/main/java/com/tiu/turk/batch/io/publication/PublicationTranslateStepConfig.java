package com.tiu.turk.batch.io.publication;
...
import com.tiu.turk.batch.io.publication.processor.PublicationProcessorComposite;
import com.tiu.turk.batch.io.publication.writer.PublicationTranslationWriter;
...
public class PublicationTranslateStepConfig {
    private final JobRepository jobRepository;
    private final PlatformTransactionManager tx;
    private final BackOffPolicy exponentialBackoffPolicy;
    private final TaskReaderFactory taskReaderFactory;
    private final PublicationTranslationWriter publicationTranslationWriter;
    private final PublicationProcessorComposite publicationCompositeProcessor;
    private final FailureListenerFactory listenerFactory;

    @Bean
    public Step translatePublicationStep() {
        return new StepBuilder("translatePublicationStep", this.jobRepository)
                .<TranslationTaskEntity, TranslationResult>chunk(20, this.tx)
                .reader(this.taskReaderFactory.getDefaultRepositoryReader(AppModule.PUBLICATION))
                .processor((ItemProcessor<TranslationTaskEntity, TranslationResult>) this.publicationCompositeProcessor)
                .writer(this.publicationTranslationWriter)
                .faultTolerant()
